...
interface BadStateDataWeak {
     ...
     * 感度上昇
     *
     * 部位ごとに指定可能 単一数値の場合は全部位
     */
    sensitivity?: SensitivityAddition;
    /**
     * もぐら出現時間バイアス(%)
     *
     * このバッドステートを付けるもぐらの出現から隠れるまでの時間を調整する
     */
    hideSpeed?: number;
    /**
     * 遅延(ms)
     *
     * ユーザーがもぐらを叩くときにこの秒数遅延して反応するようになる
     */
    delay?: number;
    /**
     * 追加効果発動周期(ms)
     *
     * なしならバッドステート付与時一回のみ追加効果が発動する
     */
    cycle?: number;
    /** 追加効果発動確率(%) */
    prod?: number;
    ...
    /**
     * 追加効果発動時 誘発バッドステートセット名
     *
     * 進行度も指定できる 停止効果がある場合は停止後に誘発される
     */
     ...
     *
     * これとは別のバッドステートのカウントがある数以上になるとこれが有効化する（付与可能になる）
     */
    countActivate?: BadStateCountCondition[];
    /**
     * 有効化に必要なバッドステートセット付与以降起動回数
     *
     * これとは別のバッドステートの付与中の追加カウント数がある数以上になるとこれが有効化する（付与可能になる）
     */
